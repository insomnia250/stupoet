...
lstm1_output = recurrent.LSTM(LSTM1_cells,return_sequences=False,implementation = 2)(encoded_poem)   #(M,LSTM1_cells)
lstm1_output = RepeatVector(X_train.shape[1])(lstm1_output)        #(M,X_train.shape[1],LSTM1_cells)
merged = layers.Concatenate(axis=2)([lstm1_output,encoded_poem])        ##(M,X_train.shape[1],LSTM1_cells+EMBEDDING_units )
lstm2_output = recurrent.LSTM(LSTM2_cells,return_sequences=True,implementation = 2)(merged) #(M,X_train.shape[1],LSTM2_cells)
...
## 权值个数计算: 
print 'weights num', vocab_size*EMBEDDING_units + LSTM1_cells*4*(LSTM1_cells+EMBEDDING_units+1) \
		+ LSTM2_cells*4*(LSTM2_cells+(LSTM1_cells+EMBEDDING_units)+1) + LSTM2_cells*vocab_size
