...
from ngram import gener_most_posible
...
def print_pred(y_pred_softmax, sample='poly'):
	print 'pred:',
	for t,char_softmax in enumerate(y_pred_softmax):
		if sample=='poly':
			char_pred = poly_sample(char_softmax, temperature=0.3)
		else:
			char_pred = np.argsort(-char_softmax)[sample]
		print index_to_word[int(char_pred)],
	print '\n',
def write_pred(result,y_pred_softmax, sample='poly'):
	for t,char_softmax in enumerate(y_pred_softmax):
		if sample=='poly':
			char_pred = poly_sample(char_softmax, temperature=0.3)
		else:
...
# if os.path.exists("7char_pair.txt".format(cycle)):
# 	os.remove("7char_pair.txt".format(cycle))
# result = codecs.open("7char_pair.txt".format(cycle),"a","utf-8")  
...
# 	most_prob_result,most_prob_proba = gener_most_posible(x,y_pred_softmax,Ncand,Nprob)
# 	for n in range(Nprob):
# 		result.write(str(most_prob_proba[n])+': ')
# 		for char in most_prob_result[n]:
# 			result.write(char)
# 		result.write( '\n')
# 	write_pred(result,y_pred_softmax, sample=0)
# 	write_pred(result,y_pred_softmax, sample=1)
# 	write_pred(result,y_pred_softmax, sample=2)
# 	write_pred(result,y_pred_softmax, sample=3)
# result.close()
...
Ncand = 5
Nprob = 5
query = [u'苟利国家生死以',u'金沙水拍云崖暖',u'盈羞浅醉流绯露',u'洞庭波涌连天雪',u'红旗卷起农奴戟',u'',u'云横九派浮黄鹤']
...
for i,x in enumerate(X_test):
	...
	print '\n',
	y_pred_softmax = Y_pred[i] 
	most_prob_result,most_prob_proba = gener_most_posible(x,y_pred_softmax,Ncand,Nprob)
	for n in range(Nprob):
		print most_prob_proba[n],':',
		for char in most_prob_result[n]:
			print char,
	print '\n'
	# y_pred_softmax = Y_pred[i] 
	print_pred(y_pred_softmax, sample=0)
	print_pred(y_pred_softmax, sample=1)
	print_pred(y_pred_softmax, sample=2)
	print_pred(y_pred_softmax, sample=3)
	print_pred(y_pred_softmax, sample=4)
