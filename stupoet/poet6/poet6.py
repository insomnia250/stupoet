...
del X,Y_list
print 'creating X_roll:' ,X_train.shape
X_roll_train = np.asarray([[word_to_index[start_token]] + list(x[0:6]) for x in X_train])
...
# modeling
poems = layers.Input(shape=(X_train.shape[1],))     #(M,X_train.shape[1])
roll_poems = layers.Input(shape=(X_train.shape[1],))     #(M,X_train.shape[1])

Embed_layer = Embedding(vocab_size,EMBEDDING_units)
encoded_poem = Embed_layer(poems)  #(M,X_train.shape[1],EMBEDDING_units)
encoded_roll_poem = Embed_layer(roll_poems)  #(M,X_train.shape[1],EMBEDDING_units)
...
merged = layers.Concatenate(axis=2)([lstm1_output,encoded_poem,encoded_roll_poem])        ##(M,X_train.shape[1],LSTM1_cells*2+EMBEDDING_units*2 )
lstm2 = recurrent.LSTM(LSTM2_cells,return_sequences=True,implementation = 2)
lstm2_output = lstm2(merged) #(M,X_train.shape[1],LSTM2_cells)
...
model = Model([poems,roll_poems],preds)
model.compile(optimizer='adam',
              loss='categorical_crossentropy',
             )
## 权值个数计算: 
print 'weights num', vocab_size*EMBEDDING_units + LSTM1_cells*4*(LSTM1_cells+EMBEDDING_units+1)*2 \
		+ LSTM2_cells*4*(LSTM2_cells+(LSTM1_cells+EMBEDDING_units)*2+1)*2 + LSTM2_cells*vocab_size
...
for cycle in range(0,8):
	if cycle<lastsave:
		...
		model.fit([X_train,X_roll_train], Y_train,
		          batch_size=2048,
		          epochs=10,
		...
		for i,x in enumerate(X_test):
			...
			input_poem = np.zeros( (1,X_test.shape[1]) )
			input_poem[0] = x
			roll_poem = np.zeros( (1,X_test.shape[1]) )
			...
			for j,word_idx in enumerate(x):
				...
				# char_pred = np.argsort(-poem_pred_softmax[0][j])[1] # 次大值
