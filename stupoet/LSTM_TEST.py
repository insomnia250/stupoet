...
vocab_freq = sorted(vocab_freq.iteritems(),key=lambda d:d[1],reverse=True)[0:100]
...
lstm_output = recurrent.LSTM(LSTM_cells,return_sequences=True)(encoded_poem)   #(M,X_train.shape[1],LSTM_cells)
...
for cycle in range(0,4):
	...
	print X_test
	poem_pred_softmax = model.predict(X_test)
	for i,x in enumerate(X_test):
		...
		print '\n-------pred------',
		result.write('\n-------pred------')
		input_vec = np.zeros( X_test.shape[1] )
		char_pred = word_to_index[start_token]
		for j,word_idx in enumerate(x):
			if word_idx!=0:
				input_vec[j] = word_idx
			else:
				input_vec[j] = char_pred
			# char_pred = np.argmax(poem_pred_softmax,axis=2)[i][j]  #vectorize
			# # sample type
			char_pred = sample(poem_pred_softmax[i][j], temperature=0.8)
			print index_to_word[int(input_vec[j])],
			result.write( index_to_word[int(input_vec[j])] +u' ')
			if j==8 : result.write(u'||')
